...
bool isSafe(int row, int col, int n, vector<int>& queen) {
    for (int i = 0; i < row; i++) {
        ...
        */
        ...
        if (placedcol == col || abs(i - row) == abs(placedcol - col)) {
            return false;
        }
    }
...
bool solution(int n, vector<int>& queen) {
    stack<pair<int,int>> stk; // declaring a stack to store pair of ints (row, col)
    int row = 0, col = 0; // initializing row and col to be 0
    ...
    while (true) {
        ...
        */
            if (isSafe(row, col, n, queen)) { // only runs if the position (row,col) is safe to place a queen
                ...
                queen[row] = col; // keeping track of the queen
                row++; // updating row value
                col = 0; // starting next iteration from first col
                placed = true;
                break;
            }
            col++;
        ...
        if (placed) {
            ...
            int prevcol = top.second; // declaring col position of last placed queen
...
void PrintSol(int n, const vector<int>& queen) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (queen[i] == j) {
                cout << "Q ";
...
int main() {
    ...
    vector<int> queen(n, -1); // declaring vector for col position of queens, starting at -1 (before the board)
    if (solution(n, queen)) {
        PrintSol(n, queen);
    } else {
